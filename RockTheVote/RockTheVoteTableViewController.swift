...
class RockTheVoteTableViewController: UITableViewController {
    ...
    func loadVote() {
        ...
        do {
            vote = try context.fetch(request)
        } catch {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let voteList = vote[indexPath.row]
        cell.textLabel?.text = voteList.songTitle!
        cell.detailTextLabel!.numberOfLines = 0
        cell.detailTextLabel?.text = "Voter: " + voteList.voterName! + "\nLike ? " + String(voteList.vote)
